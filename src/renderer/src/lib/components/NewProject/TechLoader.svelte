<script lang="ts">
  ...
  $effect(() => {
    getImageDimensions(`images/${tech.defaultIcon}`, (dimensions) => {
  ...
  $effect(() => {
    if (w) {
      anime({
        targets: `#__${tech.name}-loading-mask`,
        translateX: [-60, -20],
        easing: "linear",
        duration: 1000,
        loop: true,
  ...
  $effect(() => {
    if (h && progress) {
      anime.remove(`#__${tech.name}-loading-mask-container`)
      anime({
        targets: `#__${tech.name}-loading-mask-container`,
        ...
        update: (x) => {
          ...
          const p = ((h - v) * 100) / h
          if (document.getElementById(`__${tech.name}-progress`))
            document.getElementById(`__${tech.name}-progress`)!.innerHTML =
              `${p.toFixed(0)}%`
        },
      })
    }
  ...
  <div>
    ...
    >
      <defs>
        ...
        <mask id={`__${tech.name}-mask`} maskUnits="userSpaceOnUse">
          <g
            id={`__${tech.name}-loading-mask-container`}
            style="transform: translateY({h}px);"
          >
            <path
              id={`__${tech.name}-loading-mask`}
              ...
              fill={done ? "white" : `url(#__${tech.name}-mask-gradient)`}
            />
          </g>
        </mask>
      </defs>
      <image
        href="images/{tech.defaultIcon}"
        width={w}
        height={h}
      ...
      <image
        ...
        width={w}
        height={h}
        mask={!done ? `url(#__${tech.name}-mask)` : undefined}
      />
    </svg>
  </div>
